...
class SubjectsBloc extends Bloc<SubjectsEvent, SubjectsState> {
  SubjectsBloc() : super(SubjectsLoading()) {
    ...
    on<DeleteSubject>((event, emit) async {
      ...
      try {
        String deleteSituation =
            await SubjectServiceImp().deleteSubject(event.subjectId);
        print("in bloc add lecture");

        if (deleteSituation == "success") {
          print("success bloc delete lecture");
          emit(SuccessDeleteSubject());
        }
      } on DioException catch (e) {
        emit(FailureSubjectsState(message: e.message!));
      }
    ...
    on<AddSubject>((event, emit) async {
      ...
      try {
        ...
        if (response is DataSuccessObject<SubjectModel>) {
          print("Fetching subjects...");
          SuccessSituation responsee = await SubjectServiceImp().getSubjects();
          print(response);
