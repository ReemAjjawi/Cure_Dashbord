        ...
        return Scaffold(
          ...
          floatingActionButton: FloatingActionButton(
           ...
           onPressed: () {
              ...
              if (subjectsState is SubjectsList) {
                // Get IDs of selected subjects
                List<int> selectedSubjectIds = selectedItems
                    .asMap()
                    .entries
                    .where((entry) => entry.value) // Checkboxes marked true
                    .map((entry) => subjectsState.subjects[entry.key].id) // Map to subject IDs
                    .toList();

                // Debug: Print selected IDs
                print("Selected IDs: $selectedSubjectIds");

                // Dispatch an event
                context.read<CodeBloc>().add(
                  GenerateCode(
                    numberOfCodes: 100,
                    subjectsNumber: selectedSubjectIds,
                  ),
                );
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text("Subjects not loaded yet")),
                );
              }
            },
